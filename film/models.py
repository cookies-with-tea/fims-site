...
class People(models.Model):
    ...
    date_of_birth = models.PositiveIntegerField('Дата рождения', null=True, blank=True)
...
class FilmAbstract(models.Model):
    ...
    age = models.PositiveSmallIntegerField('Возростное ограничение', default=0,)
    time = models.FloatField('Время показа', null=True, blank=True)
...
class SuggestedFilm(FilmAbstract):

    owner = models.ForeignKey(User, verbose_name='Автор предложения', on_delete=models.CASCADE,)
    is_published = models.BooleanField('Опубликовать в релиз', default=False,)
    genre = models.ManyToManyField(Genre, verbose_name='Жанры фильма', blank=True, )
    starring = models.ManyToManyField(People, verbose_name='Главные актеры', related_name='starring_userfilm', blank=True, )
    awards = models.ManyToManyField(Award, verbose_name='Награды фильма', blank=True, )
